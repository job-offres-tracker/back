...
- `Offre.provenance` is a **free-text `String`**, not an enum — `"FRANCE_TRAVAIL"` for synced offers (the `Offre.Builder` default), anything the caller wants for manual ones. Don't confuse it with `EtatOffre` (the closed enum for reading/processing status only: `NON_LU, LU, POSTULE, REFUSE` — `POSTULE` just means a `Candidature` exists; its own outcome is tracked separately via `StatutCandidatureOffre`/`StatutCandidatureSpontanee`/`StatutPriseDeContact` on the `Candidature`, editable via `PATCH /api/v1/candidatures/{id}/statut`).
